...
from .smtp_service import create_email_template, send_email
...
def get_or_create_chat_state(username: str) -> ChatState:
    if username not in chat_sessions:
        ...
        logger.warning(
            f"Chat state not found for user {username}, creating default. Flow might be incorrect."
        )
...
async def post_chat_message(
    ...
    try:
        ...
        if current_state.stage == "start" and not current_state.history:
            ...
            logger.warning(
                f"User {username} sent message but chat not initialized. Responding with error."
            )
            ...
            raise HTTPException(
                status_code=400,
                detail="Chat not properly started. Please refresh or restart.",
            )

    except KeyError:
        # This handles the case where get_or_create_chat_state logic fails or isn't robust
        logger.error(
            f"Critical state error: Chat state missing for logged-in user {username}"
        )
        raise HTTPException(
            status_code=500, detail="Chat session error. Please login again."
        )

    # Handle the message using the Gemini chat logic
    try:
        ai_response, updated_state = await gemini_chat.handle_chat_message(
            user_id=username, user_message=user_message, current_state=current_state
        )
        ...
        logger.error(
            f"Error handling chat message for user {username}: {e}", exc_info=True
        )
        # Don't save state if there was a major error during processing
        raise HTTPException(status_code=500, detail="Error processing message.")


@app.post("/chat/emergency_contact")
async def emergency_contact_email(
    data: dict = Body(...),
    current_user: UserInDB = Depends(auth.get_current_active_user),
):
    """
    Sends an emergency contact email with the user's assessment results.
    """
    try:
        user_name = data.get(
        ...
        sender_email = "mahaveer86619.dev@gmail.com"  # Hardcoded sender
        html_content = create_email_template(
            user_name, condition, severity, recommendations
        )
        send_email(subject, sender_email, receiver_email, html_content)
        return {"status": "sent"}
    except Exception as e:
        logger.error(f"Failed to send emergency email: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to send emergency email.")
